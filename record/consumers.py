    ...
    def search(self,user,lowestid):
        ...
        try:
            ...
            if self.type == "pipeline":
                q = q.filter(machine__pipeline=self.dbobject)
            elif self.type == "garage":
                q = q.filter(machine__pipeline__garage=self.dbobject)
            else:
                q = q.filter(machine__pipeline__garage__factory=self.dbobject)
